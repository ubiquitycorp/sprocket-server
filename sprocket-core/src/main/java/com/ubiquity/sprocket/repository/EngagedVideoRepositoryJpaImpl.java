...
EngagedVideoRepository {
	...
	public List<EngagedVideo> findMeanByGroup(String group, Integer limit) {
		Query query = getEntityManager().createQuery("select ei from EngagedItem ei where ei.videoContent is not null and ei.user in (select gm.user from GroupMembership gm where gm.groupIdentifier = :group) group by ei.videoContent.videoContentId order by count(*) desc");
