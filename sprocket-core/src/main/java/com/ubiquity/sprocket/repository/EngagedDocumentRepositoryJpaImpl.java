...
EngagedDocumentRepository {
	...
	public List<EngagedDocument> findMeanByGroup(String group, Integer limit) {
		Query query = getEntityManager().createQuery("select ei from EngagedItem ei where ei.documentDataType is not null and ei.user in (select gm.user from GroupMembership gm where gm.groupIdentifier = :group) group by ei.documentDataType, ei.activity.activityId, ei.videoContent.videoContentId order by count(*) desc");
